...
elseif has('win32')
    if empty(glob('%userprofile%\.vim\autoload\plug.vim'))
      silent !curl -fLo \%userprofile\%\.vim\autoload\plug.vim --create-dirs https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim
      autocmd VimEnter * PlugInstall --sync | source $MYVIMRC
    endif
endif
...
Plug 'junegunn/fzf.vim' " Super nice Fuzzy Finder 
...
" BACKGROUND FILES
silent execute '!mkdir "'.$HOME.'/.vim/temp"'
